...
生成AIツールの個人プランを利用する際には、学習モデル利用の有無が気になるところです。ツールによっては入力データがモデルの改善に利用される設定がデフォルトになっていることがあります。そこで、この記事では、個人で生成AIツールを使う際に、入力データの扱いがどこで定義されているのかの確認ポイントを具体例を交えて紹介します。
